#!/usr/bin/env python3
"""Fail-closed software-independence audit for exact frozen V7 observers.

This checks only the implementation boundary supporting epistemic separation:
PolliPi analysis source must not import InsePi's ``interaction_sensing`` package,
and InsePi source must not import PolliPi's ``pollipi_analysis`` package. It does
not claim statistical independence of observer errors.
"""
...
def main() -> None:
    ...
    violations = [
        *audit_tree(args.pollipi_root / "packages" / "analysis" / "src", "interaction_sensing"),
        *audit_tree(args.insepi_root / "src", "pollipi_analysis"),
    ]
    if violations:
        raise RuntimeError(
            "frozen observer cross-implementation import boundary failed:\n"
            + "\n".join(violations)
        )
    print("V7_FROZEN_OBSERVER_IMPLEMENTATION_INDEPENDENCE PASS")
    print("V7_POLLIPI_FORBIDDEN_IMPORT interaction_sensing")
    print("V7_INSEPI_FORBIDDEN_IMPORT pollipi_analysis")
    print("V7_STATISTICAL_ERROR_INDEPENDENCE_CLAIM false")
