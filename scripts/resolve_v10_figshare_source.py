...
def normalise_file(row: object) -> dict[str, object]:
    ...
    return {
        ...
        "is_video": name.lower().endswith(VIDEO_SUFFIXES),
    }
...
def main() -> None:
    ...
    files = sorted((normalise_file(row) for row in files_raw), key=lambda row: (str(row["name"]).lower(), int(row["id"])))
    ...
    encoded = (json.dumps(resolution, sort_keys=True, indent=2, ensure_ascii=False) + "\n").encode("utf-8")
    resolution["resolution_sha256_without_self_field"] = hashlib.sha256(encoded).hexdigest()
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(resolution, sort_keys=True, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    ...
    print("V10_TOTAL_VIDEO_BYTES", sum(int(row["size_bytes"]) for row in videos))
    for row in videos:
        print("V10_VIDEO", row["id"], row["size_bytes"], row["name"], row["computed_md5"])
